...
public class MITH_Game_Board extends JLayeredPane {

		private MITH_Game game;
		private MITH_Player player;
		private MITH_Move move;
...
//    JPanel deckPanel;
    ...
    public MITH_Game_Board(int num_players) {
...
//							System.out.println(game);
	              ...
	              // replace the removed panel with a new one
	              	...
	              	dragLabel.card = game.drawFromDeck();
...
//            		System.out.println(target);
            		...
            		// need to check with game to make sure it's a valid move
            			...
            			if(!placeCardInRoom(playerPanel, (MITH_Room_Panel) target)) {
            				addToHand();
            			};
            		}
            		...
            		{
            			if(!addARoom((MITH_Player_Panel) target)) {
            				addToHand();
            			}
	          		}
        ...
        private void removeFromHand(MITH_Label card) {
        	...
        	// also remove it from the player's actual hand
        ...
        private void addToHand() {
        	...
        	handPanel.add(dragLabel);
       		player.giveCard(dragLabel.card);
         ...
         ***************************************************/
        private boolean addARoom(MITH_Player_Panel playerPanel) {
        	boolean validMove = false;
  				if(dragLabel.card.getType() == MITH_Card.ROOMEMPTY) {
     				move.card = dragLabel.card;
    				System.out.println(move.card.getType());
    				move.house = playerPanel.id - 1;
//    				move.roomslot = roomPanel.id - 1;
 						validMove = game.playRoomEmpty(move);
  				}
        ...
        /****************************************************
         ...
         that card's image
         ****************************************************/
        private boolean placeCardInRoom(MITH_Player_Panel playerPanel, MITH_Room_Panel roomPanel) {
...
//    					case MITH_Card.ROOMEMPTY: validMove = game.playRoomEmpty(move); break;
    	...
    	frame.getContentPane().add(new MITH_Game_Board(4));
